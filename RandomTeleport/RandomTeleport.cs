...
namespace RandomTeleport
{
    public class RandomTeleport : Mod,IGlobalSettings<GlobalSettings>,ICustomMenuMod,ITogglableMod
    {
       ...
       public Dictionary<string, List<string>> RelatedScenes = new Dictionary<string, List<string>>()
        {
        ...
        public List<string> NotAreaRoomPrefix = new List<string>  { "Room", "Dream", "Ruins", "Grimm" };

        private List<string> getSameAreaScenes(string scene, List<string> availableTeleportScenes)
        {
        ...
        private IEnumerator TeleportCoro()
        {
            ...
            if (settings.sameAreaTeleport)
            {
                ...
                {
                    Instance.Log($"Cannot execute teleport because error occured: {e.Message}");
